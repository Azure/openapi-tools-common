...
export const objectInfoSymbol = Symbol.for("./source-map/object-info")
...
export const stringMapMap = <T extends Data, R extends Data>(
    ...
    const result = sm.buildMap(source, (v, k) => {
        const r = f(v, k)
        copyDataInfo(v, r)
        return r
    })
    if (sm.isMapEqual(source, result)) {
        return source as any
    }
    copyInfo(source, result)
    return result
...
export const propertySetMap = <T extends sm.PartialStringMap<keyof T & string, Data>>(
    ...
    if (sm.isMapEqual(sm.toStringMap(source), sm.toStringMap(result))) {
        return source as any
    }
