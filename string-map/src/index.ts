...
export const entryKey = <T>(e: Entry<T>): string => e[EntryIndex.Key];

export const entryValue = <T>(e: Entry<T>): T => e[EntryIndex.Value];
...
export const entries = <T>(
  input: StringMap<T> | undefined | null
): _.IterableEx<Entry<T>> => objectEntries<string, T>(input);
...
export const keys = <T>(
  input: StringMap<T> | undefined | null
): _.IterableEx<string> => entries(input).map(entryKey);

export const values = <T>(
  input: StringMap<T> | undefined | null
): _.IterableEx<T> => entries(input).map(entryValue);
...
): StringMap<T> => stringMap(_.map(a, entries).flatMap((v) => v));
...
export const isEmpty = <T>(a: StringMap<T>): boolean => entries(a).isEmpty();
